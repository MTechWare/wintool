        ...
        firstLogonItems.forEach((item, index) => {
            ...
            if (command && command.trim()) {
                firstLogonCommands.push({
                    order: index + 1,
                    command: command.trim(),
                    description: description || `FirstLogon Command ${index + 1}`
                });
            }
        });
    }
...
function initializeTests() {
    ...
    tester.addTest('Theme Toggle', () => {
        try {
            const originalTheme = document.documentElement.getAttribute('data-theme');
            toggleTheme();
            const newTheme = document.documentElement.getAttribute('data-theme');
            toggleTheme(); // Switch back
            return originalTheme !== newTheme;
        } catch (error) {
            errorHandler.logError(error, 'Theme Toggle Test');
            return false;
        }
