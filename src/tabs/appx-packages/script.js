...
class AppXPackageManager {
  ...
  filterPackages() {
    ...
    Object.keys(this.packages).forEach(key => {
      ...
      if (this.currentFilter === 'microsoft') {
        include =
          include &&
          (pkg.publisher.toLowerCase().includes('microsoft') || pkg.name.toLowerCase().includes('microsoft'));
  ...
  async uninstallSelectedPackages() {
    ...
    const packageNames = packagesToUninstall.map(key => this.packages[key]?.name || key).join(', ');
