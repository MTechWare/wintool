...
function initServicesTab() {
  ...
  const container =
    document.querySelector('[data-tab="folder-services"]') || document.querySelector('.services-container') || document;
...
function applyFilters(container) {
  filteredServices = allServices.filter(service => {
    ...
    if (currentFilter.search) {
      ...
      return service.Name.toLowerCase().includes(searchTerm) || service.DisplayName.toLowerCase().includes(searchTerm);
    }
...
function showLoading(container, show) {
  ...
  const servicesContainer =
    container.querySelector('.services-list-section') || document.querySelector('.services-list-section');
  ...
  try {
    if (window && window.electronAPI) {
      ...
      if (savedServices && Array.isArray(savedServices)) {
        ...
        quickAccessServices = allServices.filter(service => savedServices.includes(service.Name));
      } else {
        // Use default services
        quickAccessServices = allServices.filter(service => defaultCommonServiceNames.includes(service.Name));
      }
    } else {
...
function openCustomizeModal(container) {
  ...
  availableServicesForModal = allServices.filter(service => !quickAccessServices.some(qa => qa.Name === service.Name));
