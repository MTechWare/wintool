...
// Performance optimization: Cache frequently used IPC calls
const ipcCache = new Map();
const CACHE_DURATION = 5000; // 5 seconds

function cachedInvoke(channel, ...args) {
  ...
  promise.then(result => {
    ipcCache.set(key, { result, timestamp: Date.now() });
  });
...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  getErrorLogFilePath: () => ipcRenderer.invoke('get-error-log-file-path'),
  ...
  getAutoDetectedLevel: () => ipcRenderer.invoke('get-auto-detected-level'),
  resetAutoDetection: () => ipcRenderer.invoke('reset-auto-detection'),
