...
class TabLoader {
  ...
  waitForInitialization() {
    ...
    let timeout = 2000; // Reduced default from 4 to 2 seconds for faster startup
    if (this.systemCapabilities === 'low-end') {
      timeout = 3000; // Reduced from 6 to 3 seconds
    } else if (this.systemCapabilities === 'high-end') {
      timeout = 1000; // Reduced from 2 to 1 second for high-end systems
    }
    ...
    const intervalId = setInterval(() => {
      ...
      // Early completion check - if most tabs are ready and we've waited a reasonable time
      if (elapsedTime >= 500 && this.initializedTabsCount >= Math.floor(this.totalTabs * 0.8)) {
        console.log(`Early completion: ${this.initializedTabsCount}/${this.totalTabs} tabs ready after ${elapsedTime}ms`);
        const uninitializedTabs = Array.from(this.tabReadyCallbacks.keys());
        uninitializedTabs.forEach(tabId => {
          console.warn(`Force marking tab as ready (early completion): ${tabId}`);
  ...
  async loadTab(folderName) {
    try {
      ...
      if (js.trim()) {
        ...
        const shouldDeferExecution = this.lazyLoadingEnabled && tabId !== 'welcome' && folderName !== 'welcome';
