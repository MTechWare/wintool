...
async function loadCurrentSettings() {
  try {
    if (window.electronAPI) {
      ...
      // Lazy loading setting moved to Performance Settings section
...
async function loadPerformanceSettings() {
  try {
    if (window.electronAPI) {
      ...
      // Load lazy loading setting (moved to Performance Settings)
      ...
      if (enableLazyLoadingCheckbox) {
        // Set lazy loading based on performance mode
        if (currentPerformanceMode === 'low-end') {
          // Enable lazy loading for low-end systems (faster startup)
          enableLazyLoadingCheckbox.checked = true;
          if (!enableLazyLoading) {
            await window.electronAPI.setSetting('enableLazyLoading', true);
            console.log('Auto-enabled lazy loading for Low-End Mode');
          }
        } else if (currentPerformanceMode === 'high-end') {
          // Disable lazy loading for high-end systems (all content loads immediately)
          enableLazyLoadingCheckbox.checked = false;
          if (enableLazyLoading) {
            await window.electronAPI.setSetting('enableLazyLoading', false);
            console.log('Auto-disabled lazy loading for High-End Mode');
          }
        } else {
          // Auto mode - use saved setting or default to true
...
async function savePerformanceSettings() {
  try {
    if (window.electronAPI) {
      ...
      // Save lazy loading setting (moved to Performance Settings)
...
function setupPerformanceModeHandlers() {
  ...
  modeCards.forEach((card, index) => {
    ...
    card.addEventListener('click', () => {
      ...
      const wasLazyLoadingEnabled = lazyLoadingCheckbox ? lazyLoadingCheckbox.checked : true;

      if (currentPerformanceMode === 'low-end') {
        // Fast System Information setting removed - always use detailed scanning
        document.getElementById('enable-discord-rpc').checked = false;
        document.getElementById('enable-lazy-loading').checked = true; // Enable for low-end systems

        // Show notification if lazy loading was automatically enabled
        if (!wasLazyLoadingEnabled) {
          setTimeout(() => {
            showNotification('Lazy Loading enabled for Low-End Mode - faster startup performance', 'info');
          }, 500);
        }
      } else if (currentPerformanceMode === 'high-end') {
        // Fast System Information setting removed - always use detailed scanning
        document.getElementById('enable-discord-rpc').checked = true;
        document.getElementById('enable-lazy-loading').checked = false; // Disable for high-end systems

        // Show notification if lazy loading was automatically disabled
        if (wasLazyLoadingEnabled) {
          setTimeout(() => {
            showNotification('Lazy Loading disabled for High-End Mode - all content loads immediately', 'info');
          }, 500);
        }
      } else if (currentPerformanceMode === 'auto') {
        // Auto mode - keep lazy loading enabled by default for better startup performance
...
async function showPerformanceRecommendation() {
  ...
  // If auto mode is selected, show the current auto-detected level
  if (currentPerformanceMode === 'auto') {
    try {
      const autoDetectedLevel = await window.electronAPI.getAutoDetectedLevel();
      if (autoDetectedLevel) {
        let levelDescription = '';
        switch (autoDetectedLevel) {
          case 'low-end':
            levelDescription = 'Low-End Mode (optimized for limited resources)';
            break;
          case 'high-end':
            levelDescription = 'High-End Mode (all features enabled)';
            break;
          default:
            levelDescription = 'Balanced Mode (mid-range optimizations)';
        }

        recommendationText.innerHTML = `
          Auto mode has detected and is using: <strong>${levelDescription}</strong><br>
          <button id="reset-auto-detection" class="btn btn-sm btn-outline-warning mt-2">
            🔄 Reset Auto-Detection
          </button>
          <small class="text-muted d-block mt-1">Reset to re-detect your system on next startup</small>
        `;

        // Add event listener for reset button
        const resetButton = document.getElementById('reset-auto-detection');
        if (resetButton) {
          resetButton.addEventListener('click', async () => {
            try {
              const success = await window.electronAPI.resetAutoDetection();
              if (success) {
                recommendationText.innerHTML = `
                  <span class="text-success">✅ Auto-detection reset successfully!</span><br>
                  <small class="text-muted">Your system will be re-detected on next startup.</small>
                `;
                setTimeout(() => {
                  showPerformanceRecommendation(); // Refresh the display
                }, 3000);
              } else {
                recommendationText.innerHTML = `
                  <span class="text-danger">❌ Failed to reset auto-detection</span>
                `;
              }
            } catch (error) {
              console.error('Error resetting auto-detection:', error);
              recommendationText.innerHTML = `
                <span class="text-danger">❌ Error resetting auto-detection</span>
              `;
            }
          });
        }
      ...
      console.error('Error getting auto-detected level:', error);
    }
  }

  // Hide recommendation for non-auto modes
  ...
  try {
    if (window.electronAPI && window.electronAPI.getSystemInfo) {
      ...
      const memoryGB = systemInfo.totalMemory
        ? Math.round(parseInt(systemInfo.totalMemory.replace(/[^\d]/g, '')) / 1024 / 1024 / 1024)
        : 4;
...
// Preset Visibility Settings Functions
