...
import { initStartupOptimizer } from './modules/startup-optimizer.js';

async function initialBoot() {
  const startTime = performance.now();
  console.log('🚀 WinTool starting critical boot...');

  // Disable animations during startup for faster rendering
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  // Window visibility monitoring has been removed for better performance

  await initialBoot();
  // Ultra-fast startup: Start deferred boot immediately
  deferredBoot(); // Remove timeout for immediate execution
});
...
async function continueNormalStartup() {
  ...
  try {
    ...
    if (window.TabLoader) {
      ...
      // Reduced default from 100ms to 10ms for faster startup - system auto-detection will optimize further
      const sequentialLoadDelay = await window.electronAPI.getSetting('sequentialLoadDelay', 10);
      ...
      newTabLoader.setCompleteCallback(async loadedTabs => {
        ...
        // Re-enable animations after startup is complete
        setTimeout(() => {
        ...
        // Performance analysis (recommendations disabled by user request)
        // const recommendations = getStartupRecommendations(totalStartupTime);
        // Console-based performance recommendations disabled

        if (totalStartupTime < 5000) {
          console.log(`🎉 Excellent startup performance! (${totalStartupTime.toFixed(2)}ms)`);
        } else {
          console.log(`✅ Startup performance: ${totalStartupTime.toFixed(2)}ms`);
        }
      });
      ...
      setTimeout(hideSplashScreen, 200); // Reduced from 500ms to 200ms
    }
