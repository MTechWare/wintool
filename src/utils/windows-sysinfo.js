...
// Timeout wrapper for PowerShell commands
...
class WindowsSystemInfo {
    ...
    // Get cached data or execute command
   ...
   // CPU information
    ...
    // CPU temperature (requires additional tools, fallback to null)
            ...
            try {
                ...
                try {
                    ...
                    if (Array.isArray(allGpus)) {
                        ...
                        const realGpus = allGpus.filter(gpu =>
                            gpu.Name &&
                        ...
                        if (realGpus.length > 0) {
                            ...
                            data = realGpus.sort((a, b) => (b.AdapterRAM || 0) - (a.AdapterRAM || 0))[0];
                        } else {
                    ...
                    console.warn('First GPU detection method failed:', error.message);
                }
                ...
                if (!data || !data.Name) {
                    try {
                        ...
                        console.warn('Second GPU detection method failed:', error.message);
                    }
                }
                ...
                if (data?.Name) {
                    ...
                    if (name.includes('nvidia') || name.includes('geforce') || name.includes('quadro') || name.includes('rtx') || name.includes('gtx')) {
                        ...
                        vendor = 'AMD';
                    } else if (name.includes('intel') || name.includes('uhd') || name.includes('iris') || name.includes('hd graphics')) {
                        vendor = 'Intel';
                    } else {
                ...
                if (data?.CurrentHorizontalResolution && data?.CurrentVerticalResolution) {
                    ...
                    try {
                        ...
                        if (resData?.CurrentHorizontalResolution && resData?.CurrentVerticalResolution) {
                ...
                return {
                    controllers: [{
                        ...
                        model: model,
                        bus: '',
                        vram: data?.AdapterRAM ? Math.round(data.AdapterRAM / (1024 * 1024 * 1024)) : null,
                        vramDynamic: false,
                        vramText: vramText
                    }],
                    displays: [{
                ...
                console.warn('Graphics information not available:', error.message);
                return {
                    controllers: [{ vendor: 'Unknown', model: 'Unknown', vram: null, vramText: 'Unknown' }],
                    displays: [{}],
    ...
    async osInfo() {
        return this.getCached('osInfo', async (timeout) => {
            ...
            return {
                ...
                servicepack: data.ServicePackMajorVersion ? `SP${data.ServicePackMajorVersion}` : '',
                uefi: null,
    ...
    async mem() {
        try {
            ...
            console.warn('PowerShell memory info failed, using Node.js fallback:', error.message);
    ...
    async diskLayout() {
        return this.getCached('diskLayout', async (timeout) => {
            ...
            const output = await execPowerShell(command, timeout);
            const data = Array.isArray(JSON.parse(output)) ? JSON.parse(output) : [JSON.parse(output)];
    ...
    async fsSize() {
        return this.getCached('fsSize', async (timeout) => {
            ...
            return data.map(disk => ({
                ...
                use: disk.Size ? Math.round(((parseInt(disk.Size) - parseInt(disk.FreeSpace)) / parseInt(disk.Size)) * 100) : 0,
                mount: disk.DeviceID || '',
                rw: true
            }));
        });
    ...
    async networkInterfaces() {
        try {
            ...
            console.warn('PowerShell network interfaces failed, using Node.js fallback:', error.message);
    ...
    async networkStatsCumulative() {
        try {
            ...
            console.warn('Network adapter list failed:', error.message);
            return [];
        }
    }
    ...
    // Get all system information (equivalent to systeminformation's full data)
    async getAllSystemInfo() {
        try {
            ...
            return {
                ...
                cpuCurrentSpeed: cpuCurrentSpeed.status === 'fulfilled' ? cpuCurrentSpeed.value : {},
                cpuTemperature: cpuTemperature.status === 'fulfilled' ? cpuTemperature.value : { main: null, cores: [], max: null },
                ...
                graphics: graphics.status === 'fulfilled' ? graphics.value : { controllers: [], displays: [], driverVersion: 'Unknown', resolution: 'Unknown' },
                time: await this.time(),
                timestamp: new Date().toISOString()
            };
        } catch (error) {
            console.error('Error getting comprehensive system info:', error);
            throw error;
        }
    }
}
